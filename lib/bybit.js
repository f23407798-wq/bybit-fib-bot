...
class BybitClient {
  ...
  async _fetchJSONWithProxyFallback(url) {
    ...
    for (const wrap of PUBLIC_PROXIES) {
      try {
        const data = await this._fetchJSON(wrap(url));
        // A proxy can return HTTP 200 with valid-but-wrong JSON (its own
        // error page, rate-limit notice, etc). Only accept a response that
        // actually looks like a Bybit V5 payload — otherwise keep trying
        // the remaining proxies instead of surfacing garbage as success.
        if (data && typeof data.retCode !== "undefined") return data;
        errors.push(`unexpected shape: ${JSON.stringify(data).slice(0, 150)}`);
      } catch (e) {
        errors.push(e.message);
      }
    }
    throw new Error(`All ${PUBLIC_PROXIES.length} fetch routes failed for ${url}:\n` + errors.map((m, i) => `  [${i}] ${m}`).join("\n"));
  }
  ...
  async _publicGet(path, params = {}) {
    ...
    if (data.retCode !== 0) {
      ...
      throw new Error(`Bybit ${path} error ${data.retCode}: ${data.retMsg} — raw response: ${snippet}`);
    }
    return data.result;
  }
